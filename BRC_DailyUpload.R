...
file_list  <- list.files(path="DailyReportMails", pattern="*.csv")
names <- paste(substr(file_list,1,3),substr(file_list,23,26),sep="")

for(i in 1:length(file_list)){
  assign(names[i], read.csv(paste("DailyReportMails/",file_list[i],sep=""),
                            skip=9))
}
...
for(i in 1:length(df_list) ){
  ...
  colnames(removeUnwanted) <- c("day","downtonelev","carpenterelev","setondamelev","terzaghiflow","setonsyphons","setonriverflow","setonrivercfs",
  ...
  removeNA$downtonelev <- as.numeric(removeNA$downtonelev) # Because there were some phone numbers in the file these were factors
  removeNA$carpenterelev <- as.numeric(removeNA$downtonelev)
  newList[[i]] <- removeNA
}
